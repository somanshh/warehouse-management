...
const WarehouseDetails = ({ warehouse }) => {
  ...
  if (editMode) {
    return (
      <form onSubmit={handleSubmit} className="warehouse-details edit-mode">
        ...
        <select
          name="is_live"
          value={editedWarehouse.is_live.toString()}
          onChange={handleInputChange}
        >
        ...
        <div className="edit-buttons">
          <button type="button" onClick={addCustomField} className="secondary">
            Add Custom Field
          </button>
  ...
  return (
    <div className="warehouse-details">
      ...
      <p>
        <strong>Registered:</strong> {warehouse.is_registered ? "Yes" : "No"}
      </p>
      <p>
        <strong>Live:</strong> {warehouse.is_live ? "Yes" : "No"}
      </p>
      {Object.entries(customFields).map(([key, value]) => (
        <p key={key}>
